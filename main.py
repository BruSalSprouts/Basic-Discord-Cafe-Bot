...
menu = [
]  #List of items with their prices. Easy access and can be added/deleted at any time
userItems = {
}  #Dictionary so each each user can be found a bit more easily. key:value = username:list of user's items
...
bot = commands.Bot(
    command_prefix='!',
    intents=intents)  #Only messages that start with ! will be accepted
...
async def waitress(ctx):  #The café's greeting message essentailly
    ...
    if isOpen:
        ...
        await ctx.send(
            'Hello there! The café is currently closed, come back later!')


@bot.command()
async def open(
        ctx
):  #Opens the café and changes isOpen to True. Only mods can do this
    if ctx.channel.id != targetChannelID:  #Checks to see if the command is being called in the correct channel ID
        return
...
async def close(
        ctx
):  #Closes the café and changes isOpen to false. Only mods can do this
    if ctx.channel.id != targetChannelID:  #Checks to see if the command is being called in the correct channel ID
        return
    if adminRoleID not in [role.id for role in ctx.author.roles
                           ]:  #Checks to see if user has the admin role
    ...
    imagePath = os.path.join(os.getcwd(),
                             'CafeOperations/Sorry-We-Are-Closed-PNG-Image.png'
                             )  #Gets path of image file relative to directory
    #Checks to see if the image file exists
    if not os.path.exists(imagePath):
        await ctx.send('Image file not found.')
...
@bot.command()
async def table(
        ctx):  #Displays a table and has little messages before and after
    if ctx.channel.id != targetChannelID:  #Checks to see if the command is being called in the correct channel ID
        return
    if not isOpen:  #Checks to see if the café is currently open
        await ctx.send(
            'I\'m sorry, but you\'ll have to come back later! The café is currently closed!'
        )
        return
    ...
    #Image Sending Time
    imagePath = os.path.join(os.getcwd(),
                             'CafeOperations/Dining-Room-Table-PNG-File.png'
                             )  #Gets path of image file relative to directory
    #Checks to see if the image file exists
    if not os.path.exists(imagePath):
        await ctx.send('Image file not found.')
    ...
    await ctx.send('Let me know when you\'re ready to order!'
                   )  #Closing Message
...
async def tip(
    ctx
):  #Prompts user to ask how much they want to tip, receives a numeric response, and then gives a gratitude message responding to the amount tipped
    if ctx.channel.id != targetChannelID:  #Checks to see if the command is being called in the correct channel ID
        return
    if not isOpen:  #Checks to see if the café is currently open
        await ctx.send(
    ...
    await ctx.send(
        'How much would you like to tip? (Please send respond only with digits/numbers)'
    )  #Send a message to the user asking for a tip
